...
class EditVideoController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($uploadedImage->getError() === UPLOAD_ERR_OK) { //se a chave de erro na variavel imagem for igual UPLOAD_ERR_OK (0), eu envio a imagem
            ...
            if (is_string($mimeType) && str_starts_with($mimeType, 'image/')) {
                $safeFileName = uniqid('upload_') . '_' . pathinfo($uploadedImage->getClientFilename(), PATHINFO_BASENAME); //esse pathinfo pegando o base name pega só o nome básico do arquivo mais a extenão, sem pegar o caminho completo das pastas
